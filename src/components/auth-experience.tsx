...
export function AuthExperience({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  return (
    <main id="main-content" className="morphic-auth-shell">
      <section
        className="morphic-auth-story"
        aria-labelledby="morphic-auth-title"
      >
        <Brand className="morphic-auth-brand" />
        <div className="morphic-auth-copy">
          <p>Repository facts in. Reviewable work out.</p>
          <h1 id="morphic-auth-title">
            Your next objective,
            <br />
            <span>with its evidence attached.</span>
          </h1>
          <small>
            Connect GitHub to build a workspace from live repository evidence.
            Morphic keeps decisions visible and waits for approval before Codex
            runs.
          </small>
        </div>
        <AuthRoutePreview />
      ...
      <section
        className="morphic-auth-form"
        aria-label={isSignUp ? "Create Morphic account" : "Sign in to Morphic"}
      >
        <div className="morphic-auth-mobile-brand">
          <Brand />
        </div>
        <p className="morphic-auth-form-kicker">
          {isSignUp
        ...
        <Link href="/" className="morphic-auth-back">
          ← Back to Morphic overview
        </Link>
      </section>
    </main>
  );
